...
class SnakeGameAI:
    ...
    def get_vision(self, window_size=5):
        ...
        for dy in range(-half, half+1):
            for dx in range(-half, half+1):
                # Use block_size multiples for the grid cells.
    ...
    def get_state(self):
        ...
        # Get vision window (e.g., 5x5 grid => 25 features)
        vision = self.get_vision(window_size=5)
        # Concatenate base state with vision features
        return np.concatenate((base_state, vision))
    ...
    def step(self, action):
        ...
        # ---- Compute Distance to Food (for shaping reward)
...
def train():
    ...
    # NOTE: The model's input size is now 45 (20 original features + 25 vision features).
    game = SnakeGameAI()
    model = SnakeAI(45, 256, 3)  # 45 input features, 256 hidden neurons, 3 actions
